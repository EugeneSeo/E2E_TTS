...
def train(rank, args, config, gpu_ids):
    ...
    if (args.optim_g == "G_only"):
        ...
        optim_ss = torch.optim.Adam(speech.parameters(), config.lr_ss, betas=config.betas, eps=config.eps)
    else: # args.optim_g = "G_and_SS"
        optim_g = torch.optim.AdamW(itertools.chain(generator.parameters(), speech.parameters()), config.lr_g, betas=[config.adam_b1, config.adam_b2])
    ...
    for epoch in range(max(0, last_epoch), args.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if (args.optim_g == "G_and_SS"):
                ...
                if scaler is None:
                    torch.nn.utils.clip_grad_norm_(speech.parameters(), config.grad_clip_thresh)
