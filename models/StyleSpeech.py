...
from models.Modules import Mish, LinearNorm, ConvNorm, Conv1dGLU, \
                    MultiHeadAttention, StyleAdaptiveLayerNorm, get_sinusoid_encoding_table, MultiHeadAttention_nonSelf
from models.VarianceAdaptor import VarianceAdaptor, VarianceAdaptor_ali
...
# Current Best Model
class StyleSpeech_attn(nn.Module):
    ''' StyleSpeech '''
    def __init__(self, config):
        super(StyleSpeech_attn, self).__init__()
        self.style_encoder = MelStyleEncoder_attn(config)
        self.encoder = Encoder(config)
        self.variance_adaptor = VarianceAdaptor(config)
        self.decoder = Decoder(config)
        self.attn_layer = MultiHeadAttention_nonSelf(config.encoder_head, config.encoder_hidden, config.style_hidden, 
                                config.style_hidden//config.encoder_head, config.style_hidden//config.encoder_head, config.dropout) 
    ...
    def forward(self, src_seq, src_len, mel_target, mel_len=None, 
        ...
        # Extract Style Vector
        style_vector, style_attn = self.style_encoder(mel_target, mel_mask)
        # Encoding
        encoder_output, src_embedded, _ = self.encoder(src_seq, style_vector, src_mask)
        
        # Variance Adaptor
        acoustic_adaptor_output, d_prediction, p_prediction, e_prediction, mel_len, mel_wmask = self.variance_adaptor(
                        encoder_output, src_mask, mel_len, mel_mask, 
                        d_target, p_target, e_target, max_mel_len)
        
        acoustic_adaptor_output, _ = self.attn_layer(acoustic_adaptor_output, style_attn)
...
class StyleSpeech_transformer(nn.Module):
    ''' StyleSpeech '''
    def __init__(self, config):
        super(StyleSpeech_transformer, self).__init__()
        self.style_encoder = MelStyleEncoder_attn(config)
        self.encoder = Encoder(config)
        self.variance_adaptor = VarianceAdaptor(config)
    ...
    def forward(self, src_seq, src_len, mel_target, mel_len=None, 
        ...
        # Deocoding
        mel_prediction, _, hidden_output = self.decoder(acoustic_adaptor_output, style_vector, style_attn, mel_mask)
        return mel_prediction, src_embedded, style_vector, d_prediction, p_prediction, e_prediction, src_mask, mel_mask, mel_len, acoustic_adaptor_output, hidden_output
...
# Alignment learning added
class StyleSpeech_ali(nn.Module):
    ''' StyleSpeech '''
    def __init__(self, config):
        super(StyleSpeech_ali, self).__init__()
        self.style_encoder = MelStyleEncoder(config)
        self.encoder = Encoder(config)
        self.variance_adaptor = VarianceAdaptor_ali(config)
        self.decoder = Decoder(config)
    ...
    def forward(self, src_seq, src_len, mel_target, mel_len=None, 
                    d_target=None, p_target=None, e_target=None, max_src_len=None, max_mel_len=None, ali_prior=None):
        src_mask = get_mask_from_lengths(src_len, max_src_len)
        mel_mask = get_mask_from_lengths(mel_len, max_mel_len) if mel_len is not None else None
        ...
        encoder_output, src_embedded, _ = self.encoder(src_seq, style_vector, src_mask)
        # Variance Adaptor
        acoustic_adaptor_output, d_prediction, p_prediction, e_prediction, mel_len, mel_mask, alin_out = self.variance_adaptor(
                        encoder_output, src_seq, mel_target, src_mask, src_len, mel_len, mel_mask, 
                        d_target, p_target, e_target, max_mel_len, ali_prior)
        # Deocoding
        mel_prediction, _, hidden_output = self.decoder(acoustic_adaptor_output, style_vector, mel_mask)
        return mel_prediction, src_embedded, style_vector, d_prediction, p_prediction, e_prediction, src_mask, mel_mask, mel_len, acoustic_adaptor_output, hidden_output, encoder_output, alin_out
...
class Decoder_transformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layer_stack = nn.ModuleList([FFTBlock_transformer(
            self.d_model, self.style_dim, self.d_inner, self.n_head, self.d_k, self.d_v, 
            self.fft_conv1d_kernel_size, self.style_dim, self.dropout) for _ in range(self.n_layers)])
    ...
    def forward(self, enc_seq, style_code, style_attn, mask):
        batch_size, max_len = enc_seq.shape[0], enc_seq.shape[1]
        # -- Prepare masks
        slf_attn_mask = mask.unsqueeze(1).expand(-1, max_len, -1)
        ...
        for i, dec_layer in enumerate(self.layer_stack):
            dec_output, dec_slf_attn = dec_layer(
                dec_output, style_code, style_attn,
                mask=mask,
                slf_attn_mask=slf_attn_mask)
            slf_attn.append(dec_slf_attn)
...
class FFTBlock_transformer(nn.Module):
    ''' FFT Block '''
    def __init__(self, d_model,d_style,d_inner,
                    n_head, d_k, d_v, fft_conv1d_kernel_size, style_dim, dropout):
        super(FFTBlock_transformer, self).__init__()
        self.slf_attn = MultiHeadAttention(
            n_head, d_model, d_k, d_v, dropout=dropout)
        self.saln_0 = StyleAdaptiveLayerNorm(d_model, style_dim)

        self.mhd_attn = MultiHeadAttention_nonSelf(
            n_head, d_model, d_style, d_k, d_v, dropout=dropout)
        self.saln_1 = StyleAdaptiveLayerNorm(d_model, style_dim)

        self.pos_ffn = PositionwiseFeedForward(
            d_model, d_inner, fft_conv1d_kernel_size, dropout=dropout)
        self.saln_2 = StyleAdaptiveLayerNorm(d_model, style_dim)

    def forward(self, input, style_vector, style_attn, mask=None, slf_attn_mask=None):
        # multi-head self attn
        slf_attn_output, slf_attn = self.slf_attn(input, mask=slf_attn_mask)
        slf_attn_output = self.saln_0(slf_attn_output, style_vector)
        if mask is not None:
        ...
        # multi-head self attn
        slf_attn_output, slf_attn = self.mhd_attn(input, style_attn, mask=slf_attn_mask)
        slf_attn_output = self.saln_1(slf_attn_output, style_vector)
        if mask is not None:
            slf_attn_output = slf_attn_output.masked_fill(mask.unsqueeze(-1), 0)

        # position wise FF
        output = self.pos_ffn(slf_attn_output)
        output = self.saln_2(output, style_vector)
        if mask is not None:
            output = output.masked_fill(mask.unsqueeze(-1), 0)
...
class MelStyleEncoder_attn(nn.Module):
    ''' MelStyleEncoder '''
    def __init__(self, config):
        super(MelStyleEncoder_attn, self).__init__()
        self.in_dim = config.n_mel_channels 
        self.hidden_dim = config.style_hidden
        self.out_dim = config.style_vector_dim
        self.kernel_size = config.style_kernel_size
